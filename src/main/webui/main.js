...
var messagePromise = fetch('/greeting')
    .then(response => {
            if (response.ok) {
...
theater
    ...
    .on('type:end, erase:end', function() {theater.getCurrentActor().$element.classList.remove('actor-content--typing');});// Once the messagePromise resolves, set up the scenes
...
messagePromise.then(message => {
    theater
        ...
        .play(); // Start the theater once everything is set up
});